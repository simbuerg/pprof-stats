...
#data <- rbind(data, polybench)
#data <- rbind(data, llvm_nt)
...
pos <- data[(as.numeric(data$cores) %in% c(1,2,3,4,5)),]
pos <- pos[pos$speedup < 50,]
pos <- pos[pos$speedup > 1.1,]
neg <- data[(as.numeric(data$cores) %in% c(1,2,3,4,5)),]
neg <- neg[neg$speedup < -1.1,]
neg <- neg[neg$speedup > -50,]
